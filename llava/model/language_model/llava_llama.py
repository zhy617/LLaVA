...
class LlamaTemporalAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        self.temporal_attn_score = torch.zeros(1, self.num_heads, self.max_total_token, self.max_total_token).cuda() # bsz == 1 for inference, we must set zero here

        self.recent_token = 16
        self.init_token = 1
        self.attn_sink_mask = torch.zeros(self.max_total_token, self.max_total_token, dtype=torch.bool).cuda()
        for i in range(self.max_total_token):
            if i < self.recent_token:
                for j in range(i):
                    self.attn_sink_mask[i, j] = True
            else:
                for j in range(self.init_token):
                    self.attn_sink_mask[i, j] = True
                for j in range(self.recent_token):
                    self.attn_sink_mask[i, i - j] = True
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.is_first_frame:
            ...
            if q_len > 1: #prefill
                self.temporal_attn_score[:, :, :kv_seq_len, :kv_seq_len] = attn_weights
            else: # decode
                self.temporal_attn_score[:, :, kv_seq_len - 1:kv_seq_len, :kv_seq_len] = attn_weights
            ...
            # TODO: We need input image difference information here
            # image_diff = torch.zeros(618, device=torch.device("cuda:0")).bool() # bool tensor
            # diff_q_len = torch.sum(image_diff)
            ...
            if q_len == 1: # decode
                ...
                # mask = torch.logical_or(self.attn_sink_mask[kv_seq_len - 1], torch.cat((image_diff, torch.zeros(self.max_total_token - image_diff.shape[0], dtype=torch.bool, device=torch.device("cuda:0")))))
                mask = self.attn_sink_mask[kv_seq_len - 1]

                attn_weights = torch.matmul(query_states, key_states[:, :, mask[:kv_seq_len], :].transpose(2, 3)) / math.sqrt(self.head_dim) # (bsz, self.num_heads, q_len, self.head_dim)
                self.temporal_attn_score[:, :, kv_seq_len - 1:kv_seq_len, mask] = attn_weights
                attn_weights = self.temporal_attn_score[:, :, kv_seq_len - 1:kv_seq_len, :kv_seq_len]
                ...
                self.temporal_attn_score[:, :, :q_len, :q_len] = attn_weights
...
class LlamaFlashAttention2(LlamaAttention):
    ...
    def _flash_attention_forward(
        ...
        if not self._flash_attn_uses_top_left_mask:
            ...
            # Remove the `query_length != 1` check once Flash Attention for RoCm is bumped to 2.1. For details, please see the comment in LlamaFlashAttention2 __init__.
...
class LlamaSdpaAttention(LlamaAttention):
    ...
    def forward(
        ...
        if output_attentions:
            # Improve this warning with e.g. `model.config.attn_implementation = "manual"` once this is implemented.
            logger.warning_once(
...
LLAMA_ATTENTION_CLASSES = {
    "eager": LlamaAttention, # LlamaTemporalAttention, 
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if past_key_values_length == 0:
            self.token_embeds[:, :inputs_embeds.size(1), :] = inputs_embeds
        else:
            # print("inputs_embeds", inputs_embeds.size())
            self.token_embeds[:, past_key_values_length, :] = inputs_embeds
...
class LlavaLlamaForCausalLM(LlamaForCausalLM, LlavaMetaForCausalLM):
    ...
    def forward(
        ...
        '''
        print("output token: ", output.attentions[0].size(-1))
        
        file_path = f'/home/tzy/Desktop/LLaVA/attn_output/attn_output_1_{output.attentions[0].size(-1)}.pth'
        if os.path.exists(file_path):
            attn_output = torch.load(file_path)
        else:
            attn_output = []
        
        for i in range(len(output.attentions)):
            attn_output.append(output.attentions[i])
        
        torch.save(attn_output, file_path)
        '''

        # print(output.past_key_values[0][0].size()) # layer, k/v; tensor(batch, head_num, length, head_dim)
    ...
    def generate(
        ...
        **kwargs,
        ...
        if self.is_first_frame == False:
            ...
            # handle prefill stage separately to get image's KV cache
            ...
            for i in range(len(self.model.layers)):
                self.current_past_key_values[i][0][:, :, :self.input_ids_length, :].copy_(prefill_output.past_key_values[i][0])
                self.current_past_key_values[i][1][:, :, :self.input_ids_length, :].copy_(prefill_output.past_key_values[i][1])
            ...
            images_tensor = inputs_embeds[:, self.input_ids_length - 576:self.input_ids_length, :]
            
            # handle all memory first
            for j in range(len(split_position)-1): # TODO: maybe we don't want last summary sentence?

                text_encode = self.token_embeds_memory[:, split_position[j] + 1 : split_position[j+1] + 1, :]     

                text_image_cos_sim = torch.matmul(text_encode[0], images_tensor[0].t())
                norms_text = torch.norm(text_encode[0], dim=1)
                norms_image = torch.norm(images_tensor[0], dim=1)
                norms_product_matrix = torch.outer(norms_text, norms_image)
                text_image_cos_sim = text_image_cos_sim / norms_product_matrix
                
                # If exists any token miss match the image, then the sentence is not useful.
                # We think a token matches the image, means the max cos simularity is greater than a threshold (nones, verbs), 
                # or the average cos simularity is greater than a threshold (other words).

                # max_values = torch.max(text_image_cos_sim, dim=1).values
                # mean_values = torch.mean(text_image_cos_sim, dim=1)           
                # memory_useful = not ((max_values < 0.0).any() or (mean_values < -0.05).any())
                sum_values = torch.sum(text_image_cos_sim)
                print(sum_values/(576*(split_position[j+1]-split_position[j])))
                memory_useful = sum_values/(576*(split_position[j+1]-split_position[j])) > 0
                
                # print(max_values)
                # print(mean_values)
                # memory_useful = random.choice([True, False])
                print(memory_useful)

                text_image_cos_sim = text_image_cos_sim.cpu().numpy()
                plt.imshow(text_image_cos_sim, cmap='coolwarm', interpolation='nearest', vmin=-0.2, vmax=0.2)
                plt.colorbar()
                plt.show()
            ...
            inputs = torch.tensor([[split_id]]).cuda() # put . to decode phase
            generated_length -= 1
            ...
            while True:
                ...
                # we don't want to end too early
                if total_length - current_input_ids_length < 100 :
            ...
            generated_length = len(current_sequences[0]) - 1 # last token don't have kv cache
    ...
    def generate_with_mem_ppl(
        ...
        **kwargs,
        ...
        if self.is_first_frame == False:
            ...
            for j in range(len(split_position)-1): # if We don't want last summary sentence(it will lead the model to stop output), can set range to range(len(split_position)-2).
                ...
                if any(x < self.threshold for x in ppl_list):
                    memory_useful = False
            ...
            '''
            memory_attn_score = memory_attn_score.cpu().numpy()
            plt.imshow(memory_attn_score, cmap='coolwarm', interpolation='nearest', vmin=-0.2, vmax=0.2)
            plt.colorbar()
            plt.show()
            '''
            ...
            if all_memory_useful == False:
                ...
                while True:
                    ...
                    # Add a prompt after memory, we simly add "The"
                    '''
                    if first_gen:
                        token_index = 1576
                        first_gen = False
    ...
    @torch.no_grad()
    def generate_with_mem_ppl_continue_gen(
        self,
        inputs: Optional[torch.Tensor] = None,
        images: Optional[torch.Tensor] = None,
        image_sizes: Optional[torch.Tensor] = None,
